...
export const handleChat = async (req, res) => {
    try {
        const { userId, message, messages, language, sessionId } = req.body;
        const targetUserId = userId || 'guest_123';
        const messageText = message || (Array.isArray(messages) && messages.length > 0 ? messages[messages.length - 1]?.content : '');
        ...
        // Safely retrieve fresh chat history if DB is available
        let chatHistory = [];
        try {
            if (userId) {
            ...
            console.warn("DB Chat history lookup skipped:", dbErr.message);
        }

        // Extract clean text reply
        const replyText = typeof aiResponse === 'string' 
            ? aiResponse 
            : (aiResponse?.reply || aiResponse?.text || aiResponse?.response || JSON.stringify(aiResponse));
