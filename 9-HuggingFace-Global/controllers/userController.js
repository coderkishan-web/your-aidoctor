...
const registerUser = async (req, res) => {
    try {
        ...
        if (!userName || !email || !password) {
            return res.status(400).json({ success: false, error: 'Missing Details', message: 'Missing Details' });
        }

        // validating email format
        if (!validator.isEmail(email)) {
            return res.status(400).json({ success: false, error: "Please enter a valid email", message: "Please enter a valid email" });
        }
        ...
        if (existingUser) {
            return res.status(400).json({ success: false, error: "Account already exists with this email", message: "Account already exists with this email" });
        }
...
const loginUser = async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(400).json({ success: false, error: "User does not exist", message: "User does not exist" });
        }
        ...
        if (isMatch) {
            ...
            res.status(400).json({ success: false, error: "Invalid credentials", message: "Invalid credentials" });
        }
    } catch (error) {
        console.log(error);
        res.status(500).json({ success: false, error: error.message, message: error.message });
...
const getProfile = async (req, res) => {
    try {
        ...
        res.json({ success: true, user: userObj, userData });
        ...
        if (imageFile) {
            ...
            if (cloudinaryConfigured) {
                try {
                    ...
                    console.log("⚠️  Cloudinary upload failed:", uploadError.message)
                }
            } else {
                console.log("⚠️  Cloudinary not configured — profile image not updated")
            }
        }
