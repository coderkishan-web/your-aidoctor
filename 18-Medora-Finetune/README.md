# DeepSeek-Medora-v4 — Fine-tuning Medora into the model weights

This folder contains everything you need to fine-tune a DeepSeek v4
(R1-Distill) model into **DeepSeek-Medora-v4** — a medical assistant
that produces Medora's structured card output, allergy guards,
...
> writes outside `18-Medora-Finetune/`. The production Medora app stays
> unchanged.
...
`adapters/deepseek-medora-v4/`, a merged FP16 model at
`adapters/deepseek-medora-v4/merged-fp16/`, and a GGUF Q4_K_M build at
`adapters/deepseek-medora-v4/gguf/` ready to serve through Ollama or
llama.cpp.
...
Today's Medora works by wrapping a generic LLM in **~700 lines of
...
- Other consumers can call the fine-tuned model directly without
  needing the Medora harness. The orchestration becomes optional polish.
...
production Medora server as a **labeler**:
...
sources/questions.jsonl (13 base)
        ...
        │     → Medora produces different answers → model learns the
...
### 4.2 Medora-as-labeler

Each augmented question is replayed against the live Medora dev server
(port 3050). Medora's existing card emitters fire — `safety_check`,
...
```
augmented question  ──►  Medora server  ──►  [card:KIND]{...}[/card] ──►  SFT training row
   (1 prompt)           (production stack)    (deterministic output)      (model learns this)
```

Why bootstrap this way? The Medora server already encodes every moat:
...
- Python 3.10+.
- Node.js (for the Medora dev server that acts as the labeler).

### 5.2 Step-by-step

```bash
# 0. Boot the Medora dev server (in a separate terminal). The dataset
#    builder POSTs to this; it MUST be reachable on $MEDORA_URL.
...
PERSISTENT_DIR=/tmp/medora
DB_PATH=/tmp/medora/medora.db
EOF
npx next dev -p 3050
...
#   tensorboard --logdir adapters/deepseek-medora-v4

# 5. Score against the benchmark suite — same 13 cases the production
#    Medora benchmark uses, so you get directly comparable numbers.
make validate
...
python3 ../benchmarks/build_dashboard.py \
  ...
  --title "DeepSeek-Medora-v4 Validation"
...
└── gguf/                        ← Quantized GGUF (~5 GB Q4_K_M for 8B)
    └── ...                      ← Serve via Ollama: ollama create medora -f Modelfile
```
...
2. **Free tier — local-ollama**: drop the GGUF into the cloud's
   ...
   `model_aliases.yaml`. Now Medora can route `qwen2.5:1.5b` requests
   to `medora-v4` first, falling back to the existing chain if the
   trained model is unavailable.

3. **Standalone — external integrations**: ship the GGUF to Medora
   customers (MediBot, 3D-Avatar, learnskillsai.com) who can run it
   on-device. No Medora orchestration server required — the model
...
│   ├── stage1-traces.jsonl            ← Medora replay traces
...
../benchmarks/scripts/stages/{run_medora,annotate_kb,format_sft}.py
                                                        ← reused pipeline stages
../benchmarks/medora_bench/                              ← reused evaluators
```
...
- **Trains on patterns, not on clinical judgement**. The fine-tuned
  model produces Medora-style cards reliably. It does not gain net-new
...
- **Locale signal needs more countries**. Current 5 covers US/EU/UK/Asia
  but misses LATAM/Africa where Medora will eventually deploy.
...
- **No multilingual training**. All examples are English. For Italian /
  ...
  language-overridden patient profiles after extending the Medora
...
production Medora benchmark uses. The fine-tune ships when:
...
The current production benchmark (server-orchestrated Medora) sits at
...
`medora-v4-ggml`. The full benchmark suite (`make benchmark` in
...
Datasets: bootstrapped from the production Medora server in
