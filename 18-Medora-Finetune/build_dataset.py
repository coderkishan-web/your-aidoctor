#!/usr/bin/env python3
"""Build the Medora fine-tuning dataset end-to-end.
...
Pipeline (each stage produces a reviewable artifact):
  ...
  data/augmented-questions.jsonl
            │
            │  4. ../benchmarks/scripts/stages/run_medora.py
            │     (live Medora replay — captures the canonical card output
...
Usage:
    ...
    python build_dataset.py --skip-medora     # skip live replay (uses
...
from scripts.stages.run_medora import run_stage1  # noqa: E402
...
async def run_pipeline(args) -> None:
    ...
    # 4. Live Medora replay
    if args.skip_medora:
        if not STAGE1.exists():
            print(f"ERROR: --skip-medora but {STAGE1} missing", file=sys.stderr)
            sys.exit(1)
        print(f"[stage 1] skipped — using existing {STAGE1}")
    else:
        base_url = os.environ.get("MEDORA_BASE_URL", "http://localhost:3050")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build Medora fine-tuning dataset")
    p.add_argument("--skip-medora", action="store_true",
