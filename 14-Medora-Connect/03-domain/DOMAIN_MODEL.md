# Medora Connect — Domain Model

This document defines the persistence layer, the normalization layer, and the connector interface for Medora Connect. Everything here is **additive**: new tables only, no destructive changes to existing schemas.
...
This is important because provider data (e.g., Withings) may include fields Medora does not yet use.

---

### `family_member_device_mappings`

Maps an external account/device/user to a Medora Family member.
...
> Convert provider-specific measurements into Medora `VitalReading` records.
...
All future connectors implement the same interface so the rest of Medora Connect can stay provider-agnostic.
...
The Medora Connect documentation lives in this folder (`14-Medora-Connect/`).
