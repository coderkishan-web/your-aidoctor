# Medora Connect — Product Specification

## 1. Main goal

Create a new backend layer called **Medora Connect** (internally `medora-connect`) that lets users optionally connect their electronic health devices and platforms to Medora, while keeping the existing Medora app fully functional and local-first.
...
The backend acts like a **health data hub** that normalizes data from many sources into a single Medora health model.
...
Every device gives data differently. Medora Connect stores everything in one common model:
...
```text
Map Withings account/device readings to Medora Family member
