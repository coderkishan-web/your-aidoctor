# Medora Connect — Withings Connector

Withings is the **first** connector in Medora Connect because a single Withings account can cover most of the vitals the Medora app already supports (weight, blood pressure, heart rate, temperature, oxygen saturation).

This document describes the Withings OAuth flow, supported devices, and how Withings data is mapped into Medora Connect's normalized model. Implementation details are intentionally kept at the design level — actual API endpoints and parameters should be confirmed against the official Withings Public API documentation at integration time.

---

## 1. Why OAuth, not Bluetooth

Medora Connect uses the official **Withings OAuth + Public API** flow. We do **not** extract data directly from Bluetooth devices.

Reasons:

- Withings devices sync to the Withings cloud automatically; the cloud is the canonical source of truth.
- OAuth gives Medora access to historical and future data without re-pairing or being near the device.
...
## 3. Withings device → Medora vital mapping
...
Withings can notify Medora that new measurements are available. The handler at `POST /api/connect/withings/webhook`:
...
Withings devices like **Body Smart** can be used by several family members under the same Withings account. Medora Connect resolves each measurement to a specific Medora Family member using `family_member_device_mappings`.
...
Each Withings measurement carries a stable provider-side identifier. Medora Connect stores it in `vital_readings.provider_measure_id` with a unique index on `(provider, provider_measure_id)`. Re-running a sync over the same window never creates duplicates.
