# Medora Connect — Integration Plan with the Existing Medora Repo

This document describes how Medora Connect plugs into the existing Medora repository **without breaking anything**. It is purely additive: new APIs, new tables, new service modules.
...
- A Medora Family design under `13-Medora-Family/` (product spec, contracts, backend architecture, security).
- A documented vitals model covering blood pressure, glucose, temperature, weight, heart rate, and oxygen saturation.

Medora Connect builds on that foundation rather than replacing any of it.
...
└── 07-integration-medora/INTEGRATION_PLAN.md
```
...
- If a user never connects an external provider, Medora behaves exactly as it does today.
...
1. Land the documentation in `14-Medora-Connect/` (this folder).
...
Medora Connect should reuse, not duplicate:

- **Auth/session** — same auth strategy as existing Medora API routes.
- **User identity** — same user IDs as the existing app.
- **Medora Family permission model** — the rules in `13-Medora-Family/` are the source of truth; `family-permission-service.ts` implements them.
...
- Existing Medora Family design: [`../../13-Medora-Family/README.md`](../../13-Medora-Family/README.md)
