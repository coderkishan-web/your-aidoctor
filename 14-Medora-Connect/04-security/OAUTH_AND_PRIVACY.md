# Medora Connect — OAuth, Security and Privacy

This document defines how Medora Connect handles OAuth tokens, audit logs, family permissions, and user controls. Privacy and consent are first-class requirements.
...
- The `state` parameter passed to provider authorization endpoints is opaque, single-use, and bound to the current Medora user/session.
...
Medora Connect is **not** a clinical product:
...
The AI assistant in the existing Medora app may help organize user-provided or prescription-provided schedules, but never recommend or modify dosing.
