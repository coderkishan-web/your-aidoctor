# Medora Pathogen — Model Architecture

Medora Pathogen is a **family of three specialist models** behind one MCP boundary:

```text
medora-pathogen-microscopy     CNN / ViT for microscopy + blood smear
medora-pathogen-chest-xray     CNN / ViT for chest radiographs
medora-pathogen-sequence       sequence transformer + k-mer ML for FASTA/FASTQ
```
...
HF Dataset:  medora/medora-pathogen-public-seed              (curated splits with license info)
HF Dataset:  medora/medora-pathogen-clinician-curated        (access-controlled, post-Phase 1)
HF Space:    medora/medora-pathogen-demo                     (public demo, no PHI)
```

Each ships with a model / dataset card following the same template as Medora Classify (intended use, out-of-scope, data sources, metrics with subgroups, calibration plots, known failure modes, last clinician review).
