# Medora Pathogen — Roadmap

A four-phase plan from research prototype to clinician-supervised decision support. Each phase has explicit goals, deliverables, and gates. Medora Pathogen is the most regulatory-sensitive Medora module, so the gates are stricter than in Medora Classify.
...
- Hugging Face artifacts:
  - `medora/medora-pathogen-public-seed`
  - `medora/medora-pathogen-microscopy` (v0)
  - `medora/medora-pathogen-chest-xray` (v0)
  - `medora/medora-pathogen-sequence` (v0)
  - `medora/medora-pathogen-demo`
...
**Goal:** deploy Medora Pathogen alongside clinicians as advisory output. Still no autonomous patient-facing role.

### Build

- Clinician dashboard inside Medora Family.
...
- Integration with Medora Classify so imaging / microbiology evidence flows back into clinical reasoning automatically.
...
- Strengthen multimodal hand-off with Medora Classify — let agents request imaging when reasoning warrants it and integrate the result into a single explanation.
...
- Strong red-flag escalation when Medora Classify accompanies the result.
