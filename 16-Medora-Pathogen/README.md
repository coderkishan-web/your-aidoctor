...
**Medora Pathogen** is the pathogen-identification and imaging-pattern layer of the Medora ecosystem. Where **Medora Classify** reasons over the patient's clinical picture (symptoms + vitals + history), **Medora Pathogen** identifies the **organism or radiological pattern itself** from images and genomic sequences:
...
> Internal name: `medora-pathogen`

---

## 1. Why this exists, and how it differs from Medora Classify

| | **Medora Classify** (15) | **Medora Pathogen** (16) |
|---|---|---|
...
| **Regulatory exposure** | Higher than text-only; advisory triage | Highest among Medora modules — image-based diagnostic AI sits squarely in SaMD territory |

The two modules are **complementary**. Medora Classify orchestrates the clinical conversation and may *recommend* a chest X-ray or microscopy; Medora Pathogen provides the specialized image/sequence interpretation; Medora Classify then re-runs with the new evidence.

---

## 2. Position in the Medora ecosystem
...
└── Medora Pathogen          (this module)
    ...
    ├── HF models
    │   ├── medora-pathogen-microscopy
    │   ├── medora-pathogen-chest-xray
    │   └── medora-pathogen-sequence
    ├── HF datasets (curated splits / model-card-only mirrors)
    └── Guideline RAG (WHO TB / WHO malaria / RSNA / IDSA pneumonia)
```

Like every Medora module since 13-Family, Medora Pathogen is **additive**:
...
Medora Pathogen: classify_chest_xray
   ...
   ↓
Result fed back into Medora Classify
   ↓
Medora Classify updates triage + explanation, includes the X-ray finding
   ↓
...
| [`08-integration-medora/INTEGRATION_PLAN.md`](08-integration-medora/INTEGRATION_PLAN.md) | How Medora Pathogen plugs into Classify, Family, Connect, and external agents. |
...
- [`../14-Medora-Connect/README.md`](../14-Medora-Connect/README.md) — vitals context.
- [`../15-Medora-Classify/README.md`](../15-Medora-Classify/README.md) — clinical reasoning module that calls Medora Pathogen for image/sequence evidence.
