# Medora Pathogen — Safety and Compliance

Medora Pathogen interprets clinical artifacts (microscopy slides, chest radiographs, genomic sequences). That puts it in the most regulatory-sensitive part of the Medora ecosystem. This document captures the safety guardrails, the privacy controls, the license posture, and the regulatory considerations the design assumes.

> WHO and FDA both stress that AI medical software must support clinicians, not replace them, with safety, transparency, governance, human oversight, and risk management. Image-based diagnostic AI is generally treated as **Software as a Medical Device (SaMD)** in major jurisdictions. Medora Pathogen is built against those principles from day one.
...
Because Medora Pathogen is called by an agentic AI, the MCP boundary enforces:
...
Medora Pathogen integrates with the Medora Family permission model defined in `13-Medora-Family/`:
...
- **Caregivers:** access is configurable, mirroring the matrix used by Medora Connect and Medora Classify.
...
Image and sequence datasets carry strict license terms. Medora Pathogen treats license compliance as a safety property:
...
Medora Pathogen is positioned as **clinical decision support**, not autonomous diagnosis. The actual classification depends on jurisdiction and on the **claims** the product makes; image-based diagnostic AI sits squarely in SaMD territory.
...
- Medora Pathogen makes **no claim of autonomous diagnosis**.
