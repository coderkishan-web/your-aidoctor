# Medora Classify — Dataset Strategy

A clinical classifier is only as trustworthy as its data. Medora Classify uses a **three-level** strategy that lets us prototype quickly with public data, then build a real, clinician-verified dataset, then progressively raise label quality over time.
...
These would only be relevant if Medora Classify ever ingests sequence data, which is **out of MVP scope**.

---

## 2. Level 2 — Real clinician-supervised dataset (production candidate)

This is the dataset that makes Medora Classify clinically credible. It is collected with clinical partners, under consent, and stored according to GDPR/HIPAA constraints (see `07-safety/SAFETY_AND_COMPLIANCE.md`).
...
- Vitals (linked from Medora Connect when available).
- Exam findings.
...
```text
medora/medora-classify-triage-dataset
```
...
Public Level-1 data is published as a **separate** dataset (`medora/medora-classify-public-seed`) so it can never be confused with the clinician-supervised set.
