# Medora Classify — Model Architecture

Medora Classify is intentionally a **hybrid** system, not a single end-to-end transformer. Pure LLMs over-confidently hallucinate clinical answers; pure tabular models can't read free text; pure rules can't generalize. The combination of all three plus retrieval is what makes the output safe **and** useful.
...
MVP supports English + Italian (and free code-switching between them). The training dataset includes Italian medical phrasing because that's Medora's primary user base. Additional languages added per release.
...
The system publishes a coherent set of artifacts on Hugging Face under the Medora organization:

```text
HF Dataset:  medora/medora-classify-triage-dataset
HF Model 1:  medora/medora-classify-symptom-extractor
HF Model 2:  medora/medora-classify-risk-model
HF Space:    medora/medora-classify-demo
```
