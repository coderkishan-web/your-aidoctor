# Medora Classify — Training Pipeline

This document describes the engineering layout of the Medora Classify training repository, the Hugging Face artifacts it produces, and the training jobs themselves. The actual training code is **not** added in this PR — only the design and contracts.

---

## 1. Repository layout

The training code lives in its own repository (suggested name `medora-classify-train`) so it can be released to Hugging Face Hub and Spaces without coupling to the main Medora app.

```text
medora-classify-train/
├── README.md
├── pyproject.toml
├── data/
...
The pipeline produces a coherent set of artifacts under the Medora organization on Hugging Face:

```text
HF Dataset:  medora/medora-classify-public-seed         # Level-1 public data
HF Dataset:  medora/medora-classify-triage-dataset      # Level-2/3, access-controlled
HF Model:    medora/medora-classify-symptom-extractor   # encoder + seq-to-JSON
HF Model:    medora/medora-classify-risk-model          # multi-head tabular classifier
HF Space:    medora/medora-classify-demo                # no-PHI demo, public seed only
```
...
Hugging Face Inference Endpoints
  ├── medora-classify-symptom-extractor (GPU for LLM, CPU/ONNX for encoder)
  └── medora-classify-explainer
