# Medora Classify — Product Specification

## 1. Vision

**Medora Classify** is a clinical-classification and triage engine exposed as an MCP server. An agentic AI (the existing Medora chatbot or any external MCP-aware agent) calls it with patient data and receives a structured, calibrated, explainable response.

It is **clinical decision support**, not autonomous diagnosis. WHO and FDA both stress that AI medical software must support — not replace — clinicians, with clear safety, transparency, governance, human oversight, and risk management. Medora Classify is designed against those principles from day one.
...
Medora Classify is a **multi-head** classifier. A single binary "virus vs bacteria" output is unsafe — it forces a decision when the responsible answer is often *"need clinician review."*
...
Clinician opens patient case in Medora Family
   ↓
Medora Connect supplies recent vitals (temp, SpO2, HR, BP, weight)
   ↓
Clinician triggers Medora Classify
   ↓
...
```text
Parent enters symptoms in Medora Family
   ↓
Medora Classify runs with conservative thresholds
   ↓
...
A successful Medora Classify deployment looks like:
...
- **Clinicians report** that Medora Classify changed their next step in a meaningful fraction of pilot cases.
