# Medora Classify — MCP Tool Contracts

Medora Classify ships as an **MCP (Model Context Protocol) server** so any MCP-aware agent — Claude, Claude Code, the existing Medora chatbot, or a third-party agent — can call it as a tool. The server exposes five tools.
...
MVP: English, Italian, mixed code-switching (the trained extractor must accept Italian medical phrasing because Medora's primary user base is Italian).
...
```json
{
  "status": "success",
  "model_versions": {
    "extractor": "medora-classify-symptom-extractor@1.2.0",
    "risk": "medora-classify-risk-model@0.8.3",
  ...
  "evidence_pointers": [
    {
      ...
      "retrieved_via": "medora-classify-rag"
    }
  ]
}
```
