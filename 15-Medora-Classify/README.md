...
**Medora Classify** is the clinical-classification and triage layer of the Medora ecosystem. It is an **additive** module that exposes a set of MCP (Model Context Protocol) tools so that an agentic AI — or any MCP-aware client, including Medora itself — can ask focused clinical questions and receive structured, calibrated, explainable answers.
...
Medora Classify gives the agent a **dedicated tool** to compensate for that. The agent collects data conversationally; Medora Classify does the clinical math, the safety rules, and the explanation.
...
## 3. Position in the Medora ecosystem
...
└── Medora Classify          (this module)
    ...
    │   └── explain_result
    ├── HF models
    │   ├── medora-classify-symptom-extractor
    │   └── medora-classify-risk-model
    ├── HF dataset
    │   └── medora-classify-triage-dataset
    └── Guideline RAG (WHO / CDC / NHS / NICE / AAP …)
```

Like Medora Connect, Medora Classify is **additive**:

- No existing file, table, or API is changed.
- It can be deleted in one operation without affecting the rest of Medora.
...
| [`08-integration-medora/INTEGRATION_PLAN.md`](08-integration-medora/INTEGRATION_PLAN.md) | How Medora Classify plugs into Medora, Medora Family, Medora Connect, and external agents. |
...
- [`../13-Medora-Family/README.md`](../13-Medora-Family/README.md) — family permission and consent model that gates access to per-member classifications.
- [`../14-Medora-Connect/README.md`](../14-Medora-Connect/README.md) — provides normalized vitals (temperature, SpO₂, heart rate, weight, blood pressure) that Medora Classify consumes as structured input.
