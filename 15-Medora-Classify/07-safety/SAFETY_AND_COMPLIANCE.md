# Medora Classify — Safety and Compliance

Medora Classify is intended for use in or alongside clinical care. It is **clinical decision support**, not autonomous medicine. This document captures the safety guardrails, the privacy controls, and the regulatory considerations that the design assumes.

> WHO and FDA both stress that AI medical software must support clinicians, not replace them, with safety, transparency, governance, human oversight, and risk management. Medora Classify is built against those principles.
...
Because Medora Classify is called by an agentic AI, the MCP boundary enforces:

- The agent **cannot** rewrite or hide red flags from the user.
- The agent **cannot** suppress the disclaimer string.
- The agent **cannot** call `classify_illness` with structured fields the user never confirmed unless the source is the Medora Connect / Medora Family system of record (e.g., a synced thermometer reading).
...
Medora Classify integrates with the Medora Family permission model defined in `13-Medora-Family/`:
...
- **Caregivers:** access is configurable, mirroring the same matrix used by Medora Connect.
...
Medora Classify is designed to be defensible if/when it crosses regulatory thresholds. The actual classification depends on jurisdiction and on the **claims** the product makes.
...
- Medora Classify makes **no claim of diagnosis**.
