...
Designed to surface Medora's structural wins (locale, allergy, brevity,
...
_SYSTEM_COLORS = {"medora": "#7c3aed", "chatgpt": "#94a3b8"}
_SYSTEM_LABELS = {"medora": "Medora", "chatgpt": "ChatGPT"}
...
def _headline_numbers(df: pd.DataFrame) -> dict[str, float]:
    ...
    medora = overall.get("medora", 0.0)
    chatgpt = overall.get("chatgpt", 0.0)
    return {
        "medora_overall": medora,
        "chatgpt_overall": chatgpt,
        "delta_points": (medora - chatgpt) * 100,
    }
...
def _per_category_bar(df: pd.DataFrame) -> go.Figure:
    ...
    fig = px.bar(
        ...
        text="score_pct",
        color_discrete_map={
            _SYSTEM_LABELS["medora"]: _SYSTEM_COLORS["medora"],
...
def _win_rate_pie(df: pd.DataFrame) -> go.Figure:
    ...
    if "medora" not in per_case.columns or "chatgpt" not in per_case.columns:
        per_case["medora"] = per_case.get("medora", 0)
        per_case["chatgpt"] = per_case.get("chatgpt", 0)
    medora_wins = int((per_case["medora"] > per_case["chatgpt"]).sum())
    chatgpt_wins = int((per_case["chatgpt"] > per_case["medora"]).sum())
    ties = int((per_case["medora"] == per_case["chatgpt"]).sum())
    fig = go.Figure(
        go.Pie(
            labels=["Medora wins", "Tie", "ChatGPT wins"],
            values=[medora_wins, ties, chatgpt_wins],
            marker=dict(colors=[_SYSTEM_COLORS["medora"], "#cbd5e1", _SYSTEM_COLORS["chatgpt"]]),
            hole=0.55,
    ...
    fig.update_layout(
        ...
        annotations=[
            dict(
                text=f"<b>{medora_wins}</b><br><span style='font-size:14px'>Medora wins</span>",
                showarrow=False,
                font=dict(size=24, color=_SYSTEM_COLORS["medora"]),
            )
        ],
    )
    return fig
...
def _failures_table(df: pd.DataFrame) -> str:
    ...
    then Medora failures (transparency builds trust).
    """
...
def build_dashboard(csv_path: Path, output_html: Path, title: str = "Medora vs ChatGPT — Medical Benchmark") -> None:
    ...
    medora_score = headline["medora_overall"] * 100
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .stat.medora .value {{ color: var(--medora); }}
...
<body>
  <div class="wrap">
    ...
    <div class="scorecard">
      <div class="stat medora">
        <div class="label">Medora overall</div>
        <div class="value">{medora_score:.1f}%</div>
      </div>
      ...
      <div class="stat delta">
        <div class="label">Medora advantage</div>
    ...
    <footer>
      Generated from <code>{csv_path.name}</code> · Medora Benchmark Suite v0.1
    </footer>
  </div>
</body>
</html>
