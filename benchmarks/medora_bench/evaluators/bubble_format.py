...
Medora-only check: did the reply use the `[bubble:type]` sectioning the
client UI splits on? Applies only to cases that set `should_use_bubbles`
and only to the Medora system (never penalizes ChatGPT, since it has no
...
class BubbleFormatEvaluator(Evaluator):
    ...
    def score_reply(self, case: Case, turn_idx: int, reply: Reply) -> EvalResult:
        if reply.system != "medora":
            ...
            return EvalResult(self.name, 1.0, True, "n/a (non-Medora system)")
