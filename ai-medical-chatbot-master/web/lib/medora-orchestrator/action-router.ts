...
export function route(
  ...
  switch (action.type) {
    ...
    case "start_new_chat":
      ...
      // never reach the server in the first place (MedoraApp.onCardAction
...
function encodeAsAssistantText(text: string): string {
  const payload = {
    id: `chatcmpl-medora-${Date.now().toString(36)}`,
    ...
    provider: "medora-orchestrator",
    model: "medora-deterministic",
  };
