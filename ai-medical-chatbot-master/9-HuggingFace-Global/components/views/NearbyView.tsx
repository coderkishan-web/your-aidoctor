...
/**
 * NearbyView — "Find pharmacies and doctors near you".
 *
 * Ported from the production web app (web/components/views/NearbyView.tsx)
 * so behaviour and design stay consistent across surfaces. The only
 * adaptations for the HuggingFace build:
 *   - hits this app's proxy at /api/nearby (web uses /api/proxy/nearby),
 *   - drops the optional "save contact" action (no contacts store here).
 *
 * Backend is the same MetaEngine Nearby Finder (OpenStreetMap / Overpass),
 * so the result shape is unchanged.
 */
...
export function NearbyView({ language }: NearbyViewProps) {
  ...
  const doSearch = useCallback(async (lat: number, lon: number) => {
    ...
    try {
      ...
      if (data.results?.length > 0) {
        ...
        setError(t("nearby_no_results", language));
      }
    } catch (e: any) {
      setError(
        e?.name === "AbortError"
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      ...
                      {r.eta_walk_min && (
                        <span className="text-xs text-ink-subtle flex items-center gap-0.5">
                          <Clock size={10} /> {r.eta_walk_min} {t("nearby_walk_min", language)}
                        </span>
                      )}
                    </div>
                    {r.phone && (
