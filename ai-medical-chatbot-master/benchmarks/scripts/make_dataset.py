...
Pipeline:
  ...
  sources/questions.jsonl  ──► [stage 1: run_medora]    ──► generated/stage1-traces.jsonl
                                                            (live Medora replay,
                                                             captures the cards
...
EXACT card-structured behaviour Medora currently enforces server-side.
After fine-tuning, the model alone can produce the same cards without
the Medora orchestration layer.
...
Usage:
    ...
    python scripts/make_dataset.py --skip-medora    # skip live replay,
...
from scripts.stages.run_medora import run_stage1
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build the Medora fine-tuning dataset")
    p.add_argument(
        "--questions",
        type=Path,
        default=QUESTIONS,
...
async def main() -> None:
    ...
    if args.skip_medora:
        if not stage1_path.exists():
            print(f"ERROR: --skip-medora but {stage1_path} doesn't exist", file=sys.stderr)
            sys.exit(1)
