...
This document analyzes the two HomePilotAI personas that seed Medora Research & Development:
...
### Medora adaptation: the **Evidence Researcher** role
...
Tool mapping (HomePilot → Medora Research):

| HomePilot Researcher | Medora Evidence Researcher |
|---|---|
...
These are first-class fields in the Medora R&D `LiteratureRecord` (see `05-domain/DOMAIN_MODEL.md`) so downstream evidence grading and filtering are deterministic, not vibes.

### What we explicitly do **not** copy

- The Researcher persona is general-purpose; Medora Evidence Researcher is biomedical-specialized and refuses out-of-domain queries with a clear message rather than improvising.
...
### Medora adaptation: the **Clinical Safety Reviewer** role

The General Doctor persona is patient-facing. In Medora Research, the **same safety posture** is repurposed to gate research output rather than answer patients. The reviewer is invoked at every R&D output boundary (hypotheses, candidate comparisons, simulation plans, publication drafts).

Tool mapping (HomePilot → Medora Research):

| HomePilot General Doctor | Medora Clinical Safety Reviewer |
|---|---|
...
The reviewer's MCP server inherits the General Doctor's safety adapter pattern: a thin Python MCP layer that enforces a policy document (the Medora R&D equivalent of `medical-ai-safety-policy.md`) on top of an upstream knowledge / NLP service.

### What we explicitly do **not** copy

- The General Doctor's *patient-facing* response surface. The Clinical Safety Reviewer in Medora Research speaks to **researchers and authors**, not patients, and never tries to triage a real person's symptoms. If a research user accidentally describes a real symptom, the reviewer routes them out of Medora Research and toward Medora Family / a clinician.

---

## 3. Combined Medora Research model
...
## 4. Persona packaging for Medora Research

Each Medora Research agent role ships as a persona package that mirrors the HomePilotAI structure:
...
This makes every Medora Research role:
