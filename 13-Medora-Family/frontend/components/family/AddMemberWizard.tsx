  ...
  return (
    <div>
      <div style={{ fontSize: 13, color: TOKENS.ink2, marginBottom: 16, lineHeight: 1.55 }}>
        How would you like to add this person? Sync with another Medora account to share records securely,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <WizardOption
          ...
          desc="Create a new profile yourself. Best for young children or anyone without a Medora account."
          selected={method === 'manual'}
          onClick={() => setMethod('manual')}
        />
        <WizardOption
          icon="syringe"
          title="Sync with another Medora account"
          badge="End-to-end encrypted"
          desc="Link an existing Medora user (adult, child, or guardian-managed). Shared records stay in sync across devices."
        ...
        <WizardOption
          ...
          desc="Send an invitation. They'll create their own Medora account and choose what to share with the family."
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, color: TOKENS.ink2, lineHeight: 1.5 }}>
          ...
          encryption. The new member can revoke access at any time from their own Medora settings.
...
function Step2Sync({
  ...
  return (
    <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 24, alignItems: 'center' }}>
          ...
          <div>
            ...
            <ol style={{ paddingLeft: 18, margin: 0, fontSize: 13, color: TOKENS.ink2, lineHeight: 1.7 }}>
              <li>Open <strong style={{ color: TOKENS.ink }}>Medora</strong> on the other person&apos;s phone or tablet</li>
      ...
      {syncSubmethod === 'code' && (
        <div style={{ textAlign: 'center', padding: '20px 0' }}>
          <div style={{ fontSize: 13, color: TOKENS.ink2, marginBottom: 16 }}>
            Share this 9-digit pair code with the other Medora user.
          </div>
          <div style={{
      ...
      {syncSubmethod === 'email' && (
        <div>
          <Field label="Their Medora account email" hint="They'll get a push notification to confirm. No email is sent.">
  ...
  return (
    <div>
      <Field label="Email address">
        <input style={inputStyle} type="email" placeholder="they-dont-have-medora@example.com"/>
      </Field>
      <div style={{ marginTop: 14 }}>
        <Field label="Personal message (optional)">
          <textarea
            style={{ ...inputStyle, minHeight: 80, resize: 'vertical' }}
            defaultValue="Hi! I'm using Medora Family to organize our health records. Join my family so we can share schedules and reminders."
          />
        </Field>
      </div>
      <div style={{ marginTop: 14 }}>
...
function Step3Permissions({ method }: { method: Method }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 13, color: TOKENS.ink2, marginBottom: 16, lineHeight: 1.55 }}>
        Choose what this member can see and do. They can change these later from their own Medora settings.
      </div>
      {perms.map((p) => (
        <label key={p.k} style={{
      ...
      <div style={{
        ...
        <div>
          ...
          Medora never shares family data with third parties.
        </div>
      </div>
    </div>
  );
