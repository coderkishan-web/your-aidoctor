...
export function ShareView({ language }: ShareViewProps) {
  ...
  const [appUrl, setAppUrl] = useState("https://youraidoctor.in");
  ...
  const SHARE_MSG = encodeURIComponent(
    `Get free AI medical advice! YourAIDoctor — 20 languages, no sign-up: ${appUrl}`,
  );
    ...
    if (typeof navigator !== "undefined" && (navigator as any).share) {
      try {
        await (navigator as any).share({
          title: "YourAIDoctor — Free AI Medical Assistant",
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 sm:p-8 pb-mobile-nav scroll-touch">
      <div className="max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
        <h2 className="text-2xl font-bold text-ink-base mb-1">Share YourAIDoctor</h2>
          ...
          {embedOpen && (
            <div className="px-4 pb-4 border-t border-line/40 animate-in fade-in slide-in-from-top-1 duration-200">
              <p className="text-xs text-ink-muted my-3">
                Copy this code to embed YourAIDoctor on any website, blog, or health portal.
              </p>
